...
function EventManager(options) { // assumed to be a calendar
	...
	//
	// Returns an object with delta info and a function to undo all operations.
	//
...
//    parseAsUTC - if there is no zone info, should we parse the input in UTC?
//    parseZone - if there is zone info, should we force the zone of the moment?
function makeMoment(args, parseAsUTC, parseZone) {
	...
	if (moment.isMoment(input)) {
		...
		if (isSingleString) {
			...
			// arrays have no timezone info, so assume ambiguous zone
			isAmbigZone = true;
		}
...
$.extend(DayGrid.prototype, {
	...
	rowStructs: null, // an array of objects, each holding info about a row's foreground event-rendering
...
$.extend(TimeGrid.prototype, {
	...
	renderDrag: function(start, end, seg) {
		...
		if (seg) { // if there is event info for this drag, render a helper event
			this.renderRangeHelper(start, end, seg);
...
View.prototype = {
	...
	updateHeight: function() {
		var calendar = this.calendar; // we poll the calendar for height info
